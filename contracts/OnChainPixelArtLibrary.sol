...
library OnChainPixelArtLibrary {
  ...
  function getColorCompression(uint256 colorCount) internal pure returns (uint256 comp) {
  ...
  function getColorCount(uint256[] memory layers) public pure returns (uint256 colorCount) {
    return (layers[0] & 0xFFF0) >> 4;
  }

  function getStartingIndex(uint256[] memory layers) internal pure returns (uint256 startingIndex) {
  ...
  function encodeColorArray(
    ...
    EncoderTracker memory tracker = EncoderTracker(
      ...
      0,
      new uint256[](
        (colors.length / (256 / (pixelCompression + getColorCompression(colorCount)))) + 1
      ),
      0,
      0,
      0,
    ...
    tracker.layerOnePackets = ((256 - metadataLength) / (tracker.packetLength) - 1);
    ...
    while (tracker.color < tracker.endingIndex) {
      ...
      while (
        tracker.color + tracker.numberOfConsecutiveColors < colors.length &&
        // conditions are in order of most likely to break loop for gas savings
        colors[tracker.color + tracker.numberOfConsecutiveColors] == colors[tracker.color] &&
        // less than or equal to?
        tracker.numberOfConsecutiveColors < tracker.maxConsecutiveColors
        // 1111 would be 2^4 - 1
      ) {
        tracker.numberOfConsecutiveColors = increment(tracker.numberOfConsecutiveColors);
      }
      ...
      tracker.layer =
        ...
        ((
          // make packet
          ((colors[tracker.color] << pixelCompression) + tracker.numberOfConsecutiveColors)
        ) <<
      ...
      if (
        ...
        (tracker.layerIndex == 0 && tracker.packet == tracker.layerOnePackets)
      ) {
    ...
    uint256[] memory reducedLayers = new uint256[](increment(tracker.layerIndex));
  ...
  function composePalettes(
    ...
    uint256[] memory composed = new uint256[](((colorCount1 + colorCount2) / 10) + 1);
    uint256 layerIndex;
    ...
    while (color < colorCount1) {
      colors[color] = (palette1[color / 10] >> ((color % 10) * 24)) & COLOR_MASK;
    ...
    while (color < colorCount1 + colorCount2) {
      colors[color] =
        (palette2[(color - colorCount1) / 10] >> (((color - colorCount1) % 10) * 24)) &
        COLOR_MASK;
  ...
  function composeLayer(
    ...
    while (tracker.pixel < totalPixels) {
      ...
      tracker.numberOfPixels =
        (layer[tracker.layerIndex] >> ((tracker.packet) * (tracker.packetLength))) &
      ...
      tracker.packet = getPacket(tracker.iterator, tracker.packetLength, tracker.layerOnePackets);
    }
  ...
  function composeLayers(
    ...
    return encodeColorArray(colors, pixelCompression, colorCount1 + colorCount2);
  }
  ...
  function uri(string memory data) external pure returns (string memory encoded) {
    return string(abi.encodePacked("data:application/json;base64,", base64Encode(bytes(data))));
  }

  function uriSvg(string memory data) external pure returns (string memory encoded) {
    return string(abi.encodePacked("data:image/svg+xml;base64,", base64Encode(bytes(data))));
  }
    ...
    for (uint256 i = 0; i < colorCount; i = increment(i)) {
      ...
      classes[i + 1] = string(
        abi.encodePacked(
          ...
          getColorClass(i),
          " { fill: #",
          toHexString((palette[paletteIndex] >> ((i % 10) * 24)) & COLOR_MASK),
          "} "
        )
      );
    }
  ...
  function render(
    ...
    while (tracker.pixel < tracker.limit) {
      ...
      uint256 numberOfPixels = (canvas[tracker.layerIndex] >>
        ((tracker.packet) * (tracker.packetLength))) & bitsToMask(tracker.pixelCompression);
